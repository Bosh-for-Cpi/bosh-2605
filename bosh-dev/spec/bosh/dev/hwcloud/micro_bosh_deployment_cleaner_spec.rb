require 'spec_helper'
require 'fog'
require 'fog/hwcloud/models/compute/images'
require 'fog/hwcloud/models/compute/servers'
require 'fog/hwcloud/models/compute/volumes'
...
module Bosh::Dev::Hwcloud
  describe MicroBoshDeploymentCleaner do
    ...
    let(:manifest) do
      instance_double(
        'Bosh::Dev::Hwcloud::MicroBoshDeploymentManifest',
    ...
    describe '#clean' do
      ...
      before { cloud.stub(hwcloud: compute) }
      let(:compute) { double('Fog::Compute::HwCloud::Real') }

      before { compute.stub(servers: servers_collection) }
      let(:servers_collection) { instance_double('Fog::Compute::HwCloud::Servers', all: []) }

      before { Logger.stub(new: logger) }
      let(:logger) { instance_double('Logger', info: nil) }

      before { compute.stub(images: image_collection) }
      let(:image_collection) { instance_double('Fog::Compute::HwCloud::Images', all: []) }

      before { compute.stub(volumes: volume_collection) }
      let(:volume_collection) { instance_double('Fog::Compute::HwCloud::Volumes', all: []) }

      it 'uses hwcloud cloud with cpi options from the manifest' do
      ...
      context 'when matching servers are found' do
        ...
        def make_md(hash)
          instance_double('Fog::Compute::HwCloud::Metadata', to_hash: hash)
        end
      end
      ...
      context 'when images exist' do
        let(:image_to_be_deleted_1) do
          instance_double('Fog::Compute::HwCloud::Image', name: 'BOSH-fake-image-1', destroy: nil)
        end

        let(:image_to_be_deleted_2) do
          instance_double('Fog::Compute::HwCloud::Image', name: 'BOSH-fake-image-2', destroy: nil)
        end

        let(:image_to_be_ignored) do
          instance_double('Fog::Compute::HwCloud::Image', name: 'some-other-fake-image', destroy: nil)
        end
      ...
      context 'when unattached volumes exist' do
        let(:volume1) do
          instance_double('Fog::Compute::HwCloud::Volume',
    ...
    describe '#clean_server' do
      let(:server) do
        instance_double('Fog::Compute::HwCloud::Server', {
