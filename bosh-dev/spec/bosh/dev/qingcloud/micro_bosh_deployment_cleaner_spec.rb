require 'spec_helper'
require 'fog'
require 'fog/qingcloud/models/compute/images'
require 'fog/qingcloud/models/compute/servers'
require 'fog/qingcloud/models/compute/volumes'
require 'bosh/dev/qingcloud/micro_bosh_deployment_cleaner'
...
module Bosh::Dev::Qingcloud
  describe MicroBoshDeploymentCleaner do
    ...
    let(:manifest) do
      instance_double(
        'Bosh::Dev::Qingcloud::MicroBoshDeploymentManifest',
    ...
    describe '#clean' do
      before { Bosh::QingCloudCloud::Cloud.stub(new: cloud) }
      let(:cloud) { instance_double('Bosh::QingCloudCloud::Cloud') }

      before { cloud.stub(qingcloud: compute) }
      let(:compute) { double('Fog::Compute::QingCloud::Real') }

      before { compute.stub(servers: servers_collection) }
      let(:servers_collection) { instance_double('Fog::Compute::QingCloud::Servers', all: []) }

      before { Logger.stub(new: logger) }
      let(:logger) { instance_double('Logger', info: nil) }

      before { compute.stub(images: image_collection) }
      let(:image_collection) { instance_double('Fog::Compute::QingCloud::Images', all: []) }

      before { compute.stub(volumes: volume_collection) }
      let(:volume_collection) { instance_double('Fog::Compute::QingCloud::Volumes', all: []) }

      it 'uses qingcloud cloud with cpi options from the manifest' do
        Bosh::QingCloudCloud::Cloud
          .should_receive(:new)
      ...
      context 'when matching servers are found' do
        ...
        def make_md(hash)
          instance_double('Fog::Compute::QingCloud::Metadata', to_hash: hash)
        end
      end
      ...
      context 'when images exist' do
        let(:image_to_be_deleted_1) do
          instance_double('Fog::Compute::QingCloud::Image', name: 'BOSH-fake-image-1', destroy: nil)
        end

        let(:image_to_be_deleted_2) do
          instance_double('Fog::Compute::QingCloud::Image', name: 'BOSH-fake-image-2', destroy: nil)
        end

        let(:image_to_be_ignored) do
          instance_double('Fog::Compute::QingCloud::Image', name: 'some-other-fake-image', destroy: nil)
        end
      ...
      context 'when unattached volumes exist' do
        let(:volume1) do
          instance_double('Fog::Compute::QingCloud::Volume',
    ...
    describe '#clean_server' do
      let(:server) do
        instance_double('Fog::Compute::QingCloud::Server', {
