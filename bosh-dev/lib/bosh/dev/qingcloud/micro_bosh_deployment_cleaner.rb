...
module Bosh::Dev::Qingcloud
  class MicroBoshDeploymentCleaner
    ...
    def clean
      ...
      cloud = Bosh::QingCloudCloud::Cloud.new(@manifest.cpi_options)

      servers_collection = cloud.qingcloud.servers

      Bosh::Retryable.new(tries: 20, sleep: 20).retryer do
        # QingCloud does not return deleted servers on subsequent calls
      ...
      # destroy all images
      cloud.qingcloud.images.all.each do |image|
        if image.name =~ /^BOSH/
      ...
      cloud.qingcloud.volumes.all.each do |volume|
    ...
    def clean_server(server)
      server.volumes.each do |volume|
        ...
        # QingCloud does not allow to delete a volume
