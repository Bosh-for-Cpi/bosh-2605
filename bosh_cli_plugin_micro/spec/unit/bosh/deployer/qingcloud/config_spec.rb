...
describe Bosh::Deployer::Config do
  ...
  it 'should contain default vm resource properties' do
    Bosh::Deployer::Config.configure('dir' => @dir, 'cloud' => { 'plugin' => 'qingcloud' })
  ...
  it 'should have qingcloud and registry object access' do
    config = Psych.load_file(spec_asset('test-bootstrap-config-qingcloud.yml'))
    config['dir'] = @dir
    Bosh::Deployer::Config.configure(config)
    qingcloud = double(Fog::Compute)
    Fog::Compute.stub(:new).and_return(qingcloud)
    ...
    cloud.respond_to?(:qingcloud).should be(true)
