...
module Bosh::Deployer
  describe InstanceManager do
    before(:each) do
      ...
      @config = Psych.load_file(spec_asset('test-bootstrap-config-qingcloud.yml'))
      @config['dir'] = @dir
      @config['name'] = "spec-#{SecureRandom.uuid}"
      @config['logging'] = { 'file' => "#{@dir}/bmim.log" }
      @deployer = Bosh::Deployer::InstanceManager.create(@config)
      @cloud = double('cloud')
      @qingcloud = double('qingcloud')
      @cloud.stub(:qingcloud).and_return(@qingcloud)
    ...
    def stub_stuff_in_discover_bosh_ip(ip, id)
      ...
      allow(@qingcloud).to receive(:servers).and_return(servers)
    ...
    it 'should create a Bosh instance' do
      ...
      spec = Psych.load_file(spec_asset('apply_spec_qingcloud.yml'))
    ...
    context 'with vm_cid missing but disk_cid present' do
      before do
        @config['name'] = 'test-micro-qingcloud'
