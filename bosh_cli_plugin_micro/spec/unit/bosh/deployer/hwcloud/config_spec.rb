...
describe Bosh::Deployer::Config do
  ...
  it 'should contain default vm resource properties' do
    Bosh::Deployer::Config.configure('dir' => @dir, 'cloud' => { 'plugin' => 'hwcloud' })
  ...
  it 'should have hwcloud and registry object access' do
    config = Psych.load_file(spec_asset('test-bootstrap-config-hwcloud.yml'))
    config['dir'] = @dir
    Bosh::Deployer::Config.configure(config)
    hwcloud = double(Fog::Compute)
    Fog::Compute.stub(:new).and_return(hwcloud)
    ...
    cloud.respond_to?(:hwcloud).should be(true)
