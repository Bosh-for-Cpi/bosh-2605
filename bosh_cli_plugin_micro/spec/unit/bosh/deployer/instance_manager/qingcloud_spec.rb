...
require 'bosh/deployer/instance_manager/qingcloud'
require 'bosh/deployer/registry'

module Bosh::Deployer
  describe InstanceManager::Qingcloud do
    subject(:qingcloud) { described_class.new(instance_manager, config, logger) }
    ...
    describe '#client_services_ip' do
      ...
      context 'when there is a bosh VM' do
        let(:instance) { instance_double('Fog::Compute::QingCloud::Server') }

        before do
          instance_manager.stub_chain(:state, :vm_cid).and_return('fake-vm-cid')
          instance_manager.stub_chain(:cloud, :qingcloud, :servers, :get).and_return(instance)
        end
