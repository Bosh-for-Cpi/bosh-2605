  ...
  class InstanceManager
    class Qingcloud
      ...
      def update_spec(spec)
        ...
        properties['qingcloud'] =
          config.spec_properties['qingcloud'] ||
            config.cloud_options['properties']['qingcloud'].dup

        properties['qingcloud']['registry'] = config.cloud_options['properties']['registry']
        properties['qingcloud']['stemcell'] = config.cloud_options['properties']['stemcell']
      ...
      def disk_size(cid)
        # QingCloud stores disk size in GiB but we work with MiB
        instance_manager.cloud.qingcloud.volumes.get(cid).size * 1024
      end

      def persistent_disk_changed?
        # since QingCloud stores disk size in GiB and we use MiB there
      ...
      def ssh_properties(properties)
        ssh_user = properties['qingcloud']['ssh_user']
        ssh_port = properties['qingcloud']['ssh_port'] || 22
        ssh_wait = properties['qingcloud']['ssh_wait'] || 60

        key = properties['qingcloud']['private_key']
        err 'Missing properties.qingcloud.private_key' unless key

        ssh_key = File.expand_path(key)
        unless File.exists?(ssh_key)
          err "properties.qingcloud.private_key '#{key}' does not exist"
        end
      ...
      def discover_client_services_ip
        if instance_manager.state.vm_cid
          server = instance_manager.cloud.qingcloud.servers.get(instance_manager.state.vm_cid)
