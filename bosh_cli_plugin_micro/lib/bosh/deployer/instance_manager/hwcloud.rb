  ...
  class InstanceManager
    class Hwcloud
      ...
      def update_spec(spec)
        ...
        properties['hwcloud'] =
          config.spec_properties['hwcloud'] ||
            config.cloud_options['properties']['hwcloud'].dup

        properties['hwcloud']['registry'] = config.cloud_options['properties']['registry']
        properties['hwcloud']['stemcell'] = config.cloud_options['properties']['stemcell']
      ...
      def disk_size(cid)
        # HwCloud stores disk size in GiB but we work with MiB
        instance_manager.cloud.hwcloud.volumes.get(cid).size * 1024
      end

      def persistent_disk_changed?
        # since HwCloud stores disk size in GiB and we use MiB there
      ...
      def ssh_properties(properties)
        ssh_user = properties['hwcloud']['ssh_user']
        ssh_port = properties['hwcloud']['ssh_port'] || 22
        ssh_wait = properties['hwcloud']['ssh_wait'] || 60

        key = properties['hwcloud']['private_key']
        err 'Missing properties.hwcloud.private_key' unless key

        ssh_key = File.expand_path(key)
        unless File.exists?(ssh_key)
          err "properties.hwcloud.private_key '#{key}' does not exist"
        end
      ...
      def discover_client_services_ip
        if instance_manager.state.vm_cid
          server = instance_manager.cloud.hwcloud.servers.get(instance_manager.state.vm_cid)
