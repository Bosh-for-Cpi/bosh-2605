...
describe Bosh::HwCloud::StemcellCreator do
  ...
  before do
    Bosh::HwCloud::AKIPicker.stub(:new => double("aki", :pick => "aki-xxxxxxxx"))
  end
  ...
  context "real" do
    ...
    it "should create a real stemcell" do
      ...
      Bosh::HwCloud::ResourceWait.stub(:for_snapshot).with(snapshot: snapshot, state: :completed)
      Bosh::HwCloud::ResourceWait.stub(:for_image).with(image: image, state: :available)
      ...
      Bosh::HwCloud::TagManager.should_receive(:tag).with(image, "Name", "stemcell-name 0.7.0")
  ...
  context "fake" do
    ...
    it "should create a fake stemcell" do
      ...
      Bosh::HwCloud::StemcellFinder.should_receive(:find_by_region_and_id).with(region, "ami-xxxxxxxx light")
      creator.fake
    end
