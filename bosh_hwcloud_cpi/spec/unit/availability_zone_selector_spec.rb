...
describe Bosh::HwCloud::AvailabilityZoneSelector do

  let(:instances) { double(HwCloud::EC2::InstanceCollection) }
  let(:instance) { double(HwCloud::EC2::Instance, :availability_zone => 'this_zone') }
  let(:us_east_1a) { double(HwCloud::EC2::AvailabilityZone, name: 'us-east-1a') }
  let(:us_east_1b) { double(HwCloud::EC2::AvailabilityZone, name: 'us-east-1b') }
  let(:zones) { [us_east_1a, us_east_1b] }
  let(:region) { double(HwCloud::EC2::Region, :instances => instances, :availability_zones => zones) }
