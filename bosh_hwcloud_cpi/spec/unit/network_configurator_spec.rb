...
describe Bosh::HwCloud::NetworkConfigurator do
  ...
  it "should raise an error if the spec isn't a hash" do
    expect {
      Bosh::HwCloud::NetworkConfigurator.new("foo")
    }.to raise_error ArgumentError
  end

  describe "#vpc?" do
    it "should be true for a manual network" do
      nc = Bosh::HwCloud::NetworkConfigurator.new("network1" => manual)
      nc.vpc?.should be(true)
    end

    it "should be false for a dynamic network" do
      nc = Bosh::HwCloud::NetworkConfigurator.new("network1" => dynamic)
  ...
  describe "#private_ip" do
    it "should extract private ip address for manual network" do
      ...
      nc = Bosh::HwCloud::NetworkConfigurator.new(spec)
  ...
  describe "network types" do
    ...
    it "should raise an error if both dynamic and manual networks are defined" do
      ...
      expect {
        Bosh::HwCloud::NetworkConfigurator.new(network_spec)
    ...
    it "should raise an error if neither dynamic nor manual networks are defined" do
      expect {
        Bosh::HwCloud::NetworkConfigurator.new("network1" => vip)
    ...
    it "should raise an error if an illegal network type is used" do
      expect {
        Bosh::HwCloud::NetworkConfigurator.new("network1" => {"type" => "foo"})
      }.to raise_error Bosh::Clouds::CloudError, "Invalid network type 'foo' for HwCloud, " \
    ...
    describe "#configure" do
      ...
      describe "with vip" do
        it "should configure dynamic network" do
          ...
          nc = Bosh::HwCloud::NetworkConfigurator.new(network_spec)
