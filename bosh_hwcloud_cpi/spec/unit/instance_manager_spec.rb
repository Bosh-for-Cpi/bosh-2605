...
describe Bosh::HwCloud::InstanceManager do
  ...
  describe "#create" do
    ...
    it "should ask HwCloud to create an instance in the given region, with parameters built up from the given arguments" do
      ...
      disk_locality = nil
      environment = nil
      options = {"hwcloud" => {"region" => "us-east-1"}}
    ...
    it "should ask HwCloud to create a SPOT instance in the given region, when resource_pool includes spot_bid_price" do
      ...
      sg1 = double(HwCloud::EC2::SecurityGroup, security_group_id:"sg-baz-1234")
      ...
      expect(Bosh::HwCloud::ResourceWait).to receive(:for_instance).with(instance: instance, state: :running)
    ...
    it "should retry checking spot instance request state when HwCloud::EC2::Errors::InvalidSpotInstanceRequestID::NotFound raised" do
  ...
  describe "setting instance parameters" do
    ...
    describe "#set_security_groups_parameter" do
      ...
      context "when the networks specs have no security groups specified" do
        it "returns the list of default HwCloud group names" do
          instance_manager = described_class.new(region, registry)
          instance_manager.set_security_groups_parameter(
    ...
    describe "#set_availability_zone_parameter" do
      let(:availability_zone_selector) { double(Bosh::HwCloud::AvailabilityZoneSelector) }
  ...
  context 'ELBs' do
    ...
    let(:elb) { double(HwCloud::ELB, :load_balancers => load_balancers) }

    before(:each) do
      allow(HwCloud::ELB).to receive(:new).and_return(elb)
