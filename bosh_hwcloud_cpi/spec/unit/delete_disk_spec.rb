...
describe Bosh::HwCloud::Cloud do
  let(:volume) { double(HwCloud::EC2::Volume, id: 'v-foo') }
  let(:cloud) do
    mock_cloud do |ec2|
  ...
  before do
    Bosh::HwCloud::ResourceWait.stub(sleep_callback: 0)
  end

  it 'deletes an EC2 volume' do
    Bosh::HwCloud::ResourceWait.stub(for_volume: {volume: volume, state: :deleted})
  ...
  it 'retries deleting the volume if it is in use' do
    ...
    volume.should_receive(:delete).once.ordered.and_raise(HwCloud::EC2::Errors::Client::VolumeInUse)
  ...
  it 'raises an error if the volume remains in use after every deletion retry' do
    ...
    volume.should_receive(:delete).exactly(10).times.and_raise(HwCloud::EC2::Errors::Client::VolumeInUse)
  ...
  it 'does a fast path delete when asked to' do
    ...
    options['hwcloud']['fast_path_delete'] = 'yes'
    ...
    Bosh::HwCloud::ResourceWait.should_not_receive(:for_volume)
