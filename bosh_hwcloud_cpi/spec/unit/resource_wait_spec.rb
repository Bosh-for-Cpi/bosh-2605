...
describe Bosh::HwCloud::ResourceWait do
    ...
    context 'creation' do
      context 'when EC2 fails to find an instance' do
        it 'should wait until the state is running' do
          instance.should_receive(:status).and_raise(HwCloud::EC2::Errors::InvalidInstanceID::NotFound)
      ...
      context 'when resource is not found' do
        it 'should wait until the state is running' do
          instance.should_receive(:status).and_raise(HwCloud::Core::Resource::NotFound)
          instance.should_receive(:status).and_return(:pending)
          instance.should_receive(:status).and_return(:running)
      ...
      it 'should fail if HwCloud terminates the instance' do
        instance.should_receive(:status).and_return(:pending)
        instance.should_receive(:status).and_return(:pending)
        instance.should_receive(:status).and_return(:terminated)
  ...
  describe '.for_attachment' do
    let(:volume) { double(HwCloud::EC2::Volume, id: 'vol-1234') }
    let(:instance) { double(HwCloud::EC2::Instance, id: 'i-5678') }
    let(:attachment) { double(HwCloud::EC2::Attachment, volume: volume, instance: instance, device: '/dev/sda1') }
    ...
    context 'attachment' do
      ...
      it 'should retry when HwCloud::Core::Resource::NotFound is raised' do
        attachment.should_receive(:status).and_raise(HwCloud::Core::Resource::NotFound)
        attachment.should_receive(:status).and_return(:attached)
    ...
    context 'detachment' do
      ...
      it 'should consider HwCloud::Core::Resource::NotFound to be detached' do
        attachment.should_receive(:status).and_return(:detaching)
        attachment.should_receive(:status).and_raise(HwCloud::Core::Resource::NotFound)
  ...
  describe '.for_volume' do
    let(:volume) { double(HwCloud::EC2::Volume, id: 'v-123') }
    ...
    context 'deletion' do
      ...
      it 'should consider InvalidVolume error to mean deleted' do
        volume.should_receive(:status).and_return(:deleting)
        volume.should_receive(:status).and_raise(HwCloud::EC2::Errors::InvalidVolume::NotFound)
  ...
  describe '.for_snapshot' do
    let(:snapshot) { double(HwCloud::EC2::Snapshot, id: 'snap-123') }
  ...
  describe '.for_image' do
    let(:image) { double(HwCloud::EC2::Image, id: 'ami-123') }
    ...
    context 'creation' do
      ...
      it 'should wait if HwCloud::EC2::Errors::InvalidAMIID::NotFound raised' do
        image.should_receive(:state).and_raise(HwCloud::EC2::Errors::InvalidAMIID::NotFound)
        image.should_receive(:state).and_return(:pending)
        image.should_receive(:state).and_return(:available)
  ...
  describe '.for_subnet' do
    let(:subnet) { double(HwCloud::EC2::Subnet, id: 'subnet-123') }
