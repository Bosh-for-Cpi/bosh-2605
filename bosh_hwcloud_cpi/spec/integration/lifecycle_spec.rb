...
describe Bosh::HwCloud::Cloud do
  ...
  # Use subject-bang because HwCloud SDK needs to be reconfigured
  # with a current test's logger before new HwCloud::EC2 object is created.
  # Reconfiguration happens via `HwCloud.config`.
  subject!(:cpi) do
    described_class.new(
      'hwcloud' => {
        'region' => 'us-east-1',
  ...
  # because HwCloud returns Malformed error
  # for instance ids that are not proper HwCloud hashed values.
