      ...
      def validate_options(options)
        ...
        case options[:swift_provider]
          ...
          when 'hwcloud'
            ...
            raise 'HwCloud key is missing' unless options[:hwcloud].has_key?(:hwcloud_key)
            raise 'HwCloud signature method is missing' unless options[:hwcloud].has_key?(:hwcloud_signature_method)
            raise 'HwCloud signature version is missing' unless options[:hwcloud].has_key?(:hwcloud_signature_version)
            raise 'HwCloud version is missing' unless options[:hwcloud].has_key?(:hwcloud_version)
            raise 'HwCloud region name is missing' unless options[:hwcloud].has_key?(:hwcloud_region_name)
            raise 'HwCloud availability zone is missing' unless options[:hwcloud].has_key?(:hwcloud_availability_zone)
          when 'openstack'
