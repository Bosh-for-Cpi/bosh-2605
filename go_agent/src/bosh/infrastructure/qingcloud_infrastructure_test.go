...
func init() {
	Describe("qingcloudInfrastructure", func() {
		...
		BeforeEach(func() {
			...
			qingcloud = NewQingcloudInfrastructure(metadataService, registry, platform, devicePathResolver, logger)
		})
		...
		Describe("GetSettings", func() {
			It("gets settings", func() {
				...
				settings, err := qingcloud.GetSettings()
		...
		Describe("SetupNetworking", func() {
			It("sets up DHCP on the platform", func() {
				...
				err := qingcloud.SetupNetworking(networks)
				Expect(err).ToNot(HaveOccurred())
			...
			It("returns error if configuring DHCP fails", func() {
				...
				err := qingcloud.SetupNetworking(boshsettings.Networks{})
				Expect(err).To(HaveOccurred())
		...
		Describe("GetEphemeralDiskPath", func() {
			It("returns the real disk path given an qingcloud hint", func() {
				platform.NormalizeDiskPathRealPath = "/dev/xvdb"
				platform.NormalizeDiskPathFound = true

				realPath, found := qingcloud.GetEphemeralDiskPath("/dev/sdb")
			...
			It("returns an empty string to indicated that there is no ephemeral disk path if device path is empty", func() {
				realPath, found := qingcloud.GetEphemeralDiskPath("")
				Expect(found).To(BeTrue())
