...
module Bosh::Agent
  class Infrastructure::Qingcloud::Registry
    class << self
      ...
      ##
      # Gets the OpenSSH public key. First we try to get it from the Qingcloud meta data endpoint, if we fail,
      ...
      def get_openssh_key
        ...
        logger.info("Failed to get OpenSSH public key from Qingcloud meta data endpoint: #{e.message}")
      ...
      ##
      # Gets the server name from Qingcloud user data.
      #
      # @return [String] Qingcloud server name
      def get_server_name
        user_data = get_user_data
        unless user_data.has_key?("server") && user_data["server"].has_key?("name")
          raise LoadSettingsError, "Cannot get Qingcloud server name from user data #{user_data.inspect}"
        end
        user_data["server"]["name"]
      end

      ##
      # Gets the Bosh registry endpoint from Qingcloud user data.
      #
      ...
      ##
      # If the Bosh registry endpoint is specified with a Bosh DNS name, i.e. 0.registry.default.qingcloud.bosh,
      ...
      #
      # @param [Hash] user_data Qingcloud user data (generated by the CPI)
      # @return [String] Bosh registry endpoint
      ...
      ##
      # Gets the Qingcloud user data. First we try to get it from the Qingcloud user data endpoint, if we fail,
      # then we fallback to the injected user data file.
      #
      # @return [Hash] Qingcloud user data
      def get_user_data
        return @user_data if @user_data
        begin
          raw_user_data = get_uri(META_DATA_URI + "/user-data")
        rescue LoadSettingsError => e
          logger.info("Failed to get user data from Qingcloud user data endpoint: #{e.message}")
          raw_user_data = get_user_data_from_file
        end

        logger.info("Qingcloud user data: #{raw_user_data.inspect}")
        @user_data = parse_user_data(raw_user_data)
      end

      ##
      # Gets the Qingcloud user data from the injected user data file.
      #
      # @return [String] Qingcloud user data
      def get_user_data_from_file
        File.read(USER_DATA_FILE)
      rescue SystemCallError => e
        raise LoadSettingsError, "Failed to get user data from Qingcloud injected user data file: #{e.message}"
      end

      ##
      # Parses the Qingcloud user data.
      #
      # @param [String] raw_user_data Raw Qingcloud user data
      # @return [Hash] Qingcloud user data
