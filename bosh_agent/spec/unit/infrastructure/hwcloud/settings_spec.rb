...
describe Bosh::Agent::Infrastructure::Hwcloud::Settings do
  let(:hwcloud_settings) { Bosh::Agent::Infrastructure::Hwcloud::Settings.new }

  describe :get_settings do
    let(:settings) { {"vm" => "test_vm", "disks" => "test_disks"} }

    it "should load settings" do
      hwcloud_settings.should_receive(:setup_openssh_key)
      Bosh::Agent::Infrastructure::Hwcloud::Registry.should_receive(:get_settings).and_return(settings)

      loaded_settings = hwcloud_settings.load_settings
  ...
  describe :setup_openssh_key do
    ...
    it "should setup the public OpenSSH key" do
      Bosh::Agent::Infrastructure::Hwcloud::Registry.should_receive(:get_openssh_key).and_return("test_key")
      hwcloud_settings.stub(:authorized_keys).and_return(test_authorized_keys)
      ...
      hwcloud_settings.setup_openssh_key
    ...
    it "should do nothing if registry doesn't returns a public OpenSSH key" do
      Bosh::Agent::Infrastructure::Hwcloud::Registry.should_receive(:get_openssh_key).and_return(nil)
  ...
  describe :get_network_settings do
    it "should raise unsupported network exception for unknown  network" do
      ...
      expect {
        network_settings = hwcloud_settings.get_network_settings("test", network_properties)
