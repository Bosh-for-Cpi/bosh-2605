...
describe Bosh::QingCloud::Cloud do
  describe 'validating initialization options' do
    it 'raises an error warning the user of all the missing required configurations' do
      ...
      }.to raise_error(ArgumentError, 'missing configuration parameters > qingcloud:region, qingcloud:default_key_name, registry:endpoint, registry:user, registry:password')
    end
